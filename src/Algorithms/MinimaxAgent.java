...
public class MinimaxAgent {
    public int[] move(int[][] board, boolean maximizingPlayer, int roundsLeft) {
        ...
        System.out.println("My turns" + turns);
        TreeNode<int[]> bestMove = calculate(root, board, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, 0, 8, maximizingPlayer, turns);
...
//        System.out.println("Best data: " + bestMove.getScore());
    ...
    public int[] move2(int[][] board, boolean maximizingPlayer, int roundsLeft) {
        TreeNode<int[]> root = new TreeNode<>(null, false);
        int turns = Utils.calculateNumMoves(board, roundsLeft);
        TreeNode<int[]> bestMove = calculate2(root, board, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, 0, 8, maximizingPlayer, turns);
//        // TO DEBUG MIN MAX TREE, UNCOMMENT THIS
//        root.printNodeScores();
//        System.out.println("Best data: " + bestMove.getScore());
    ...
    public TreeNode<int[]> calculate(TreeNode<int[]> node, int[][] board, double alpha, double beta, int depth, int maxDepth, boolean isMaximizingPlayer, int roundsLeft) {
        ...
        List<int[]> possibleMoves = Utils.getPossibleMoves(board, isMaximizingPlayer, 14);
...
//                System.out.println("Move: (" + move[0] + ", " + move[1] + ")");
    ...
    public TreeNode<int[]> calculate2(TreeNode<int[]> node, int[][] board, double alpha, double beta, int depth, int maxDepth, boolean isMaximizingPlayer, int roundsLeft) {
        ...
        List<int[]> possibleMoves = Utils.getPossibleMoves(board, isMaximizingPlayer, 13);
        // Init best move and best score
